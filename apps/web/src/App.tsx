...
function SourceView({ interaction, inventory }: { interaction: Extract<Interaction, { type: "source" }>; inventory: ProjectInventory }) {
  ...
  return <div className="workspace-card source-workspace">
    ...
    <Editor height="100%" language={languageFor(interaction.path)} value={source?.content ?? "Loading source…"} theme="vs-dark" options={{ readOnly: !interaction.editable, minimap: { enabled: false }, fontSize: 13, lineNumbersMinChars: 3, padding: { top: 16 } }} />
  </div>;
}
...
export function App() {
  ...
  const selectedInteraction = lesson?.interactions[0];
  ...
  const selectLesson = (id: string) => { setLessonId(id); setRailOpen(false); };
